...
frame delta sandbox = if collided (sandbox ^. leftBox) (sandbox ^. rightBox)
                      then (updateCount . moveLeft . moveRight
                            . updateVelocitiesAfterBounce) sandbox
...
updateBoxPosition delta boxLens sandbox =
    ...
    then updateCount $ getNextPosition delta boxLens $
...
getNextPosition delta boxLens sandbox =
    over (boxLens . xPos)
         (+ (delta * fromIntegral fps * (sandbox ^. boxLens . velocity)))
         sandbox

updateVelocitiesAfterBounce :: Sandbox -> Sandbox
updateVelocitiesAfterBounce sandbox = sandbox & (leftBox . velocity)
    .~ newLeftVelocity & (rightBox . velocity) .~ newRightVelocity
  where
...
draw sandbox =
    pictures [ color black (line [ (0, -300), (-0, 300) ])
             , translate (sandbox ^. rightBox . xPos)
                         (sandbox ^. rightBox . size / 2) $ color red $
                   rectangleSolid (sandbox ^. rightBox . size)
                                  (sandbox ^. rightBox . size)
             , translate (sandbox ^. leftBox . xPos)
                         (sandbox ^. leftBox . size / 2) $ color blue $
                   rectangleSolid (sandbox ^. leftBox . size)
                                  (sandbox ^. leftBox . size)
